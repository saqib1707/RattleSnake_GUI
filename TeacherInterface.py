...
class Login:
	...
	def getMarks(self):
		with open('StudentRecords.txt') as readStudentFile:
			jsonData2=json.load(readStudentFile)
			for i  in range(len(jsonData2)):
				if jsonData2[i]['name']==self.name_entry.get() and self.marks_entry.get()!="" and jsonData2[i]['batch']==self.batch:
					jsonData2[i]['marks']=self.marks_entry.get()
					msg="Marks Modified for %s"%self.name_entry.get()
					tkMessageBox.showinfo('Congos',msg)
	...
	def showListOfStudents(self):
		...
		with open('StudentRecords.txt') as readStudentFile:
			jsonData2=json.load(readStudentFile)
			for i in range(len(jsonData2)):
				if jsonData2[i]['batch']==self.batch:
					data={jsonData2[i]['name']:jsonData2[i]['marks']}
					listOfStudent.append(data)
			with open('/home/saqib1707/Desktop/Extra_RattleSnake_copy/temp.txt','w') as tempFile:
				json.dump(listOfStudent,tempFile,indent=4)
			os.system('xdg-open /home/saqib1707/Desktop/Extra_RattleSnake_copy/temp.txt')
	...
	def uploadMarks(self):
		...
		with open('StudentRecords.txt') as readStudentFile:
			...
			for i in range(len(jsonData2)):
				if jsonData2[i]['batch']==self.batch:
					arg='Enter marks for '+jsonData2[i]['name']+':'
		...
		with open('TeacherRecords.txt') as readTeacherFile:
			...
			for i in range(len(jsonData1)):
				if jsonData1[i]['username']==username and jsonData1[i]['password']==pwd :
					...
					"""
					text=Text(frame)
					text.insert(INSERT,jsonData1[i]['name'])
					text.insert(END,'-Batch ')
					text.insert(END,batch)
					text.insert(END,'\nAccess Granted')
					text.pack()
					"""
					...
					showList_btn=Button(main.frame,text='List students in ur Batch',command=self.showListOfStudents,activebackground='grey',activeforeground='#AB78F1',cursor='arrow',bg='#58F0AB',highlightcolor='red',padx='10px',pady='3px').place(x=40,y=260)
					uploadMarks_btn=Button(main.frame,text='Upload marks of latest test',command=self.uploadMarks,activebackground='grey',activeforeground='#AB78F1',cursor='arrow',bg='#58F0AB',highlightcolor='red',padx='10px',pady='3px').place(x=215,y=260)
					modifyMarks_btn=Button(main.frame,text='Modify marks of student',command=self.modifyMarks,activebackground='grey',activeforeground='#AB78F1',cursor='arrow',bg='#58F0AB',highlightcolor='red',padx='10px',pady='3px').place(x=400,y=260)
					break
			if self.batch==None:
...
class TeacherInterface:			
	...
	def back(self):
		...
		submit=Button(self.frame,text="Login",command=login_obj.login,activebackground='grey',activeforeground='#AB78F1',cursor='arrow',bg='#58F0AB',highlightcolor='red',padx='10px',pady='3px').place(x=250,y=100)
